...
class FolioModel extends Model
{
	...
	public function filterDates($obj)
	{
		...
		MUNICIPIOASIGNADO.MUNICIPIODESCR AS MUNICIPIOASIGNADO, TIPOEXPEDIENTE.TIPOEXPEDIENTEDESCR, EMPLEADOS.AREADESCR, TIPOEXPEDIENTE.TIPOEXPEDIENTECLAVE 
		FROM FOLIO 
	...
	public function videos_expediente_model($tipo)
	{
		if ($tipo == 1) {
			$strQuery = '
		SELECT FOLIO.*, USUARIOS.*, ROLES.*, TIPOEXPEDIENTE.*, 
		GROUP_CONCAT(DISTINCT IMPUTADO.NOMBRE) AS IMPUTADO_NOMBRE,
		GROUP_CONCAT(DISTINCT OFENDIDO.NOMBRE, " " , IF(OFENDIDO.PRIMERAPELLIDO,OFENDIDO.PRIMERAPELLIDO, "-")) AS OFENDIDO
		FROM FOLIO 
		LEFT JOIN USUARIOS ON USUARIOS.ID = FOLIO.AGENTEATENCIONID 
		...
		LEFT JOIN FOLIOPERSONAFISICA AS IMPUTADO ON IMPUTADO.FOLIOID = FOLIO.FOLIOID AND IMPUTADO.CALIDADJURIDICAID = 2
		LEFT JOIN FOLIOPERSONAFISICA AS OFENDIDO ON OFENDIDO.FOLIOID = FOLIO.FOLIOID
		WHERE EXPEDIENTEID IS NOT NULL AND AGENTEATENCIONID IS NOT NULL AND FOLIO.TIPOEXPEDIENTEID IS NOT NULL 
		AND AGENTEFIRMAID IS NOT NULL AND TIPODENUNCIA = "VD"
		AND TIPODENUNCIA = "VD" 
        AND ( OFENDIDO.CALIDADJURIDICAID = 1 OR  OFENDIDO.CALIDADJURIDICAID = 6)
		AND FOLIO.INSTITUCIONREMISIONMUNICIPIOID =' . session('MUNICIPIOID') . '
		GROUP BY FOLIO.FOLIOID;';
		} else if ($tipo == 3) {
			$strQuery = '
		SELECT FOLIO.*, USUARIOS.*, ROLES.*, TIPOEXPEDIENTE.*, 
		...
		GROUP_CONCAT(DISTINCT OFENDIDO.NOMBRE, " " ,  IF(OFENDIDO.PRIMERAPELLIDO,OFENDIDO.PRIMERAPELLIDO, "-")) AS OFENDIDO
		FROM FOLIO 
		LEFT JOIN USUARIOS ON USUARIOS.ID = FOLIO.AGENTEATENCIONID AND USUARIOS.MUNICIPIOID = FOLIO.MUNICIPIOASIGNADOID
		LEFT JOIN ROLES ON ROLES.ID = USUARIOS.ROLID 
		LEFT JOIN TIPOEXPEDIENTE ON TIPOEXPEDIENTE.TIPOEXPEDIENTEID = FOLIO.TIPOEXPEDIENTEID 
		LEFT JOIN FOLIOPERSONAFISICA AS IMPUTADO ON IMPUTADO.FOLIOID = FOLIO.FOLIOID AND IMPUTADO.CALIDADJURIDICAID = 2
		LEFT JOIN FOLIOPERSONAFISICA AS OFENDIDO ON OFENDIDO.FOLIOID = FOLIO.FOLIOID
