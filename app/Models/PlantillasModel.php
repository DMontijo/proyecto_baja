...
class PlantillasModel extends Model
{
	...
	{
		$sql = 'SELECT FD.FOLIOID, FD.TIPODOC, FD.FECHAFIRMA, FD.ANO, MU.MUNICIPIODESCR, CONCAT(US.NOMBRE, " ", US.APELLIDO_PATERNO, " ",US.APELLIDO_MATERNO) AS NOMBRE_MP, CONCAT(FPS.NOMBRE, " ", FPS.PRIMERAPELLIDO, " ",FPS.SEGUNDOAPELLIDO) AS NOMBRE_VTM, FPS.SEXO, FPS.EDADCANTIDAD, FO.HECHODELITO, FO.EXPEDIENTEID, FP.LESIONES
		FROM FOLIODOC AS FD 
	...
	{
		$sql = 'SELECT FD.FOLIOID, FD.FECHAFIRMA, FD.ANO, MU.MUNICIPIODESCR, CONCAT(US.NOMBRE, " ", US.APELLIDO_PATERNO, " ",US.APELLIDO_MATERNO) AS NOMBRE_MP, CONCAT(FPS.NOMBRE, " ", FPS.PRIMERAPELLIDO, " ",FPS.SEGUNDOAPELLIDO) AS NOMBRE_VTM,FO.EXPEDIENTEID, DELMOD.DELITOMODALIDADDESCR
		FROM FOLIODOC AS FD 
		LEFT JOIN FOLIO AS FO ON FO.FOLIOID = FD.FOLIOID
		LEFT JOIN MUNICIPIO AS MU ON MU.MUNICIPIOID = FD.MUNICIPIOID
		LEFT JOIN USUARIOS AS US ON US.ID = FD.AGENTE_REGISTRO
