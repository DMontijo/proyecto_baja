...
class PlantillasModel extends Model
{
	...
	{
		$sql = 'SELECT FD.FOLIOID, FD.TIPODOC, FD.FECHAFIRMA, FD.ANO, MU.MUNICIPIODESCR, US.NOMBRE AS NOMBRE_MP, US.APELLIDO_PATERNO AS APATERNO_MP, US.APELLIDO_MATERNO AS AMATERNO_MP, FO.HECHODELITO, FO.EXPEDIENTEID, FPS.NOMBRE, FPS.PRIMERAPELLIDO, FPS.SEGUNDOAPELLIDO, FPS.SEXO, FPS.EDADCANTIDAD, FP.LESIONES
		FROM FOLIODOC AS FD 
		LEFT JOIN FOLIO AS FO ON FO.FOLIOID = FD.FOLIOID
		LEFT JOIN MUNICIPIO AS MU ON MU.MUNICIPIOID = FD.MUNICIPIOID
		LEFT JOIN USUARIOS AS US ON US.ID = FO.AGENTEFIRMAID
		...
		WHERE FD.TIPODOC LIKE "%ORDEN DE PROTECCION%" AND MU.ESTADOID = 2 AND FPS.CALIDADJURIDICAID = 1';
