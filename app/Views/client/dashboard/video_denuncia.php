			...
			<div class="card text-center">
				<div class="card-body p-0 m-0">
					<div class="ratio ratio-16x9">
						<iframe style="min-height:600px;" src="<?= 'https://videodenunciaserver1.fgebc.gob.mx/videollamada?folio=' . $body_data->folio . '&nombre=' . $session->NOMBRE . ' ' . $session->APELLIDO_PATERNO . ' ' . $session->APELLIDO_MATERNO . '&delito=' . $body_data->delito . '&idioma=' . $body_data->idioma . '&edad=' . $body_data->edad . '&perfil=' . $body_data->perfil . '&sexo=' . $body_data->sexo . '&prioridad=' . $body_data->prioridad . '&sexo_denunciante=' . $body_data->sexo_denunciante . '&descripcion=' . $body_data->descripcion . '&random=' . uniqid() ?>" frameborder="0" allow="camera *;microphone *"></iframe>
					</div>
				</div>
			</div>
		</div>
