							...
							<tbody>
								<?php foreach ($body_data->constancia as $index => $constancia) { ?>
									<tr>
										...
										<td class="text-center"><?= date("d-m-Y", strtotime($constancia->FECHAFIRMA)) ?>
										</td>
										<td class="text-center"><?= date("H:i", strtotime($constancia->HORAFIRMA)) ?></td>
										<td class="text-center"><?= $constancia->LUGARFIRMA ?></td>
										<td class="text-center"><?= $constancia->EXTRAVIO == 'DOCUMENTOS' ? $constancia->TIPODOCUMENTO : $constancia->EXTRAVIO ?></td>
										...
										<?php if ($constancia->STATUS == 'ABIERTO') { ?>
											<td class="text-center"><a type="button" href="<?= base_url('/admin/dashboard/constancia_extravio_show?folio=' . $constancia->CONSTANCIAEXTRAVIOID . '&year=' . $constancia->ANO) ?>" class="btn btn-primary text-white"><i class="fas fa-eye"></i> VER
													SOLICITUD</a></td>
										<?php } ?>
