			...
			if ($updateExpediente->status == 201) {
				...
				if ($update) {
					...
					if ($_bandeja_creada->status == 201) {
						...
						var_dump($this->_folioDocModel->join('RELACIONFOLIODOCEXPDOC', 'FOLIODOC.NUMEROEXPEDIENTE = RELACIONFOLIODOCEXPDOC.EXPEDIENTEID  AND FOLIODOC.FOLIODOCID = RELACIONFOLIODOCEXPDOC.FOLIODOCID','LEFT')->orderBy('FOLIODOC.FOLIODOCID', 'asc')->like('FOLIODOC.TIPODOC', 'SOLICITUD DE PERITAJE')->orLike('FOLIODOC.TIPODOC', 'OFICIO DE COLABORACION PARA INGRESO A HOSPITAL')->where('FOLIODOC.NUMEROEXPEDIENTE', $expediente)->where('FOLIODOC.STATUS', 'FIRMADO'));exit;
