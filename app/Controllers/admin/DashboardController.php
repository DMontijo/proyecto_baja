		...
		if (session('ROLID') == 11 || session('ROLID') == 1 || session('ROLID')== 7) {
			...
			foreach ($data->folio as $key => $value) {
				...
				if ($value->OFICINAASIGNADOID) {
					...
					// ->where('AGENTEATENCIONID !=', null)
					// ->where('FOLIO.TIPOEXPEDIENTEID !=', null)
					// ->where('AGENTEFIRMAID !=', null)
					// ->where('TIPODENUNCIA', 'VD')
					// ->join('USUARIOS', 'USUARIOS.ID = FOLIO.AGENTEATENCIONID AND USUARIOS.MUNICIPIOID = FOLIO.MUNICIPIOASIGNADOID', 'left')
					// ->join('ROLES', 'ROLES.ID = USUARIOS.ROLID', 'left')
					// ->join('TIPOEXPEDIENTE', 'TIPOEXPEDIENTE.TIPOEXPEDIENTEID = FOLIO.TIPOEXPEDIENTEID', 'left')
					// ->where('FOLIO.OFICINAASIGNADOID', session('OFICINAID'))->findAll();
