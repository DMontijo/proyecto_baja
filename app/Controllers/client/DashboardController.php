...
class DashboardController extends BaseController
{
	...
	public function create()
	{
		...
		if ($this->request->getPost('responsable') == "SI") {
			$dataImputado = array(
		...
		if ($this->request->getPost('es_menor') == "SI") {
			$dataMenor = array(
				//DATOS MENOR
		...
		if ($this->request->getPost('delito') == "49" || $this->request->getPost('delito') == "50") {
			$dataVehiculo = array(
				// ROBO DE VEHICULO
		...
		$idmenor = $this->request->getPost('es_menor') == "SI" ? $this->_datosdelmenorModel->insert($dataMenor) : "0";
		$idadulto = $this->request->getPost('eres_tu') == "SI" ? $this->_datosdeladultoModel->insert($dataAdulto) : "0";
		$iddesaparecido = $this->request->getPost('esta_desaparecido') == "SI" ? $this->_datosdesaparecidoModel->insert($dataDesaparecido) : "0";
		$idresponsable = $this->request->getPost('responsable') == "SI" ? $this->_datosdelresponsableModel->insert($dataImputado) : "0";
		$idrobovehiculo = ($this->request->getPost('delito') == "49" || $this->request->getPost('delito') == "50") ? $this->_datosvehiculoModel->insert($dataVehiculo) : "0";
		...
		$dataFolio = [
			...
			'ID_DATOS_DEL_RESPONSABLE' => $idresponsable,
			'ID_DATOS_ADULTO_ACOMPANANTE' => $idadulto,
			'ID_DATOS_MENOR_EDAD' =>  $idmenor,
			'ID_DATOS_PERSONA_DESAPARECIDA' => $iddesaparecido,
			'ID_DATOS_ROBO_VEHICULO' => $idrobovehiculo,
		];
